...
public class DretchTests
{
    ...
    public void ForProfiling_Depth([ValueSource(nameof(Depths))] int searchDepth, [ValueSource(nameof(Teams))] Team toMove)
    {
        ...
        var initialPositions = ((Team team, FastPiece piece)[])board.positions.Clone();

        ai.GetMove(board);

        Assert.That(board.whiteKing, Is.EqualTo(new FastIndex(1, 'G')));
        Assert.That(board.blackKing, Is.EqualTo(new FastIndex(9, 'G')));
        Assert.That(board.passantableIndex, Is.EqualTo(FastIndex.Invalid));
        Assert.That(board.plySincePawnMovedOrPieceTaken, Is.EqualTo(0));
        Assert.That(board.currentMove, Is.EqualTo(toMove));
        Assert.That(board.positions, Is.EqualTo(initialPositions));
