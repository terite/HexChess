...
public class LastMoveTracker : MonoBehaviour
{
    ...
    public string GetPieceString(Piece piece) => piece switch {
        Piece.King => "King",
        Piece.Queen => "Queen",
        var p when (p == Piece.KingsKnight || p == Piece.QueensKnight) => "Knight",
        var p when (p == Piece.KingsRook || p == Piece.QueensRook) => "Rook",
        var p when (p == Piece.KingsBishop || p == Piece.QueensBishop) => "Bishop",
        var p when (p == Piece.WhiteSquire || p == Piece.GraySquire || p == Piece.BlackSquire) => "Squire",
        var p when (
